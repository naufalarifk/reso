import { IcArrowBottom, IcArrowTop, IcBitcoin, IcDropdown, IcSwapHorizontal, IcWarning } from "@/assets/icons";
        ...
        <section style={styles} className="mt-4 lg:ml-4 w-full lg:w-1/3 p-4 space-y-4">
            ...
            <div className="flex justify-between">
                <div className="flex space-x-4">
                    {
                        menu.map(menu =>
                            <Text className={`${selectedMenu === menu ? 'text-white border-b-2 border-amber-400' : ''} cursor-pointer ${isActive === 'Swap' && menu === 'TP/SL' && 'hidden'}`} onClick={() => setSelectedMenu(menu)}>{menu}</Text>)
                    }
                </div>
            </div>
            <SwapComponent />
